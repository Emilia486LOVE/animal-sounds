  ...
  <div>
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="scope">
            <el-button link type="primary" @click="handleManageAudio(scope.row)">管理音频</el-button>
            <el-button link type="primary" @click="handleEdit(scope.row)">编辑</el-button>
            <el-button link type="warning" @click="handleRefreshCount(scope.row)">刷新数量</el-button>
            <el-button link type="danger" @click="handleDelete(scope.row.datasetId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        ...
        <el-form-item>
          ...
          <el-button @click="modalVisible = false" style="margin-left: 8px">取消</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    >
      <div style="margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center">
        <div>
          <span style="margin-right: 16px">共 <strong>{{ datasetAudioList.length }}</strong> 个音频文件</span>
          <el-button size="small" type="success" @click="showUploadDialog = true">上传音频</el-button>
        </div>
        <div>
          <el-button
            size="small"
      ...
      >
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="scope">
            <el-button link type="primary" @click="handleMoveSingle(scope.row)">转移</el-button>
            <el-button link type="danger" @click="handleDeleteSingle(scope.row.audioId)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
    ...
    <!-- 上传音频对话框 -->
    <el-dialog title="上传音频文件" v-model="showUploadDialog" width="500px" append-to-body>
      <el-upload
        ref="uploadRef"
    ...
    <!-- 转移到数据集对话框 -->
    <el-dialog title="选择目标数据集" v-model="showMoveDialog" width="400px" append-to-body>
      <el-select v-model="targetDatasetId" placeholder="选择目标数据集" style="width: 100%">
        <el-option
    ...
    <!-- 批量转移到数据集对话框 -->
    <el-dialog title="批量转移到数据集" v-model="showBatchMoveDialog" width="400px" append-to-body>
...
import { getAllDatasets, createDataset, updateDataset, deleteDataset, refreshAudioCount } from '../api/dataset'
import { getAudioFilesByDataset, deleteAudioFile, moveAudioToDataset, batchMoveAudio, batchDeleteAudio } from '../api/audio'
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('删除数据集不会删除关联的音频文件，但音频将不再关联任何数据集。确认删除？', '提示', {
      type: 'warning'
    })
...
const moveableDatasets = computed(() => {
  ...
  return datasets.value.filter(d => d.datasetId !== currentDataset.value.datasetId)
})
...
const handleDeleteSingle = async (audioId) => {
  try {
    await ElMessageBox.confirm('确认删除该音频文件？删除后不可恢复。', '提示', { type: 'warning' })
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(`确认删除选中的 ${selectedAudioIds.value.length} 个音频文件？`, '提示', { type: 'warning' })
